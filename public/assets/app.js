(function () {
  ...
  function adminSettings(state) {
    ...
    return `${pageHeading('Pengaturan', 'Atur informasi yang dilihat pelanggan dan kelola menu PO.')}
      <section class="admin-panel"><div class="panel-head"><h2>Kelola toko</h2></div><div class="quick-actions"><button class="btn btn-primary" data-admin-page="menus">Menu makanan & foto</button><button class="btn btn-secondary" data-admin-page="batches">Tanggal dan kuota PO</button><button class="btn btn-ghost" data-admin-page="testimonials">Testimoni</button><button class="btn btn-ghost" data-admin-action="request-browser-notification">Aktifkan notifikasi HP</button></div></section>
      <section class="admin-panel"><div class="panel-head"><h2>📸 Upload & Ganti Foto Menu</h2><button class="btn btn-primary btn-sm" data-admin-action="add-menu">+ Tambah menu baru</button></div><div class="notice info">Tekan <strong>Ganti Foto / Edit</strong> pada menu untuk memilih foto baru dari HP/Laptop. Foto otomatis dikompresi ke WebP.</div><div class="admin-menu-grid">${state.menus.map((menu) => `<article class="admin-menu-card"><img src="${esc(menuImage(menu))}" alt="${esc(menu.imageAlt || `Foto ${menu.name}`)}"><div class="admin-menu-body"><div class="admin-menu-title"><div><span>${esc(menu.category)}</span><h3>${esc(menu.name)}</h3></div><span class="badge ${menu.active ? 'badge-open' : 'badge-closed'}">${menu.active ? 'Aktif' : 'Nonaktif'}</span></div><p>${rupiah(menu.price)} / ${esc(menu.unit)}</p><div class="admin-row-actions"><button class="btn btn-primary btn-sm" data-admin-action="edit-menu" data-menu-id="${esc(menu.id)}">📸 Ganti Foto / Edit</button><button class="btn ${menu.active ? 'btn-warning' : 'btn-success'} btn-sm" data-admin-action="toggle-menu" data-menu-id="${esc(menu.id)}">${menu.active ? 'Nonaktifkan' : 'Aktifkan'}</button></div></div></article>`).join('')}</div></section>
